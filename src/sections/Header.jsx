  ...
  return (
    <nav className="w-full bg-gray-600 flex  justify-between items-center gap-1 lg:px-16 px-6 py-4 sticky top-0 z-50">
      <h1 className="text-white md:text-5xl text-4xl font-Lobster">
        ICY <span className="text-red-600 italic">Delights</span>
      </h1>
      ...
      <button className="bg-red-600 hover:bg-white hover:text-black text-white px-10 py-3 rounded-full font-bold transform hover:scale-105 transition-transform duration-300 cursor-pointer md:flex hidden">
        ORDER NOW
      </button>
      ...
      >
        <ul className="flex flex-col justify-center items-center gap-2 w-full">
          {navItems.map(({ link, path }) => (
            <Link
              key={path}
              className="text-white uppercase font-semibold cursor-pointer p-2 rounded-lg  hover:text-white hover:bg-gray-600 w-full text-center"
              to={path}
              spy={true}
              offset={-100}
              smooth={true}
